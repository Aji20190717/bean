...
class MainPage extends Component {
    // 호출 순서 : constructor(), static getDerivedStateFromProps(), render(), componentDidMount()
    ...
    socket = io.connect('https://bean1.ml:3000', {transports:['websocket'], rejectUnauthorized:false});
    // 소켓을 만드는 코드.
    // 연결해줄? 주소, {설정명:설정값}
    // transports:['websocket'] : 순수한 웹소켓만 사용하고 싶을 때 설정
    // rejectUnauthorized:false : Certificate Authorities에 인증되지 않은 접속 거부. 기본적으로 false

    id = 0;
    opid = 0;
    name = '나';
    // 스프링이랑 연결해서 member_name 가져와서 사용하기

    state = {
        width: 0,
        height: 0,
    }
    // 어떤 컴포넌트에만 한정하여 사용되는 데이터를 포함하며, 해당 데이터는 시간이 지남에 따라 변경될 수 있습니다.

    componentDidMount() {
        var search = window.location.search;
        // url에서 정보를 가져오는 함수
        // search는 url에서 ? 뒤의 쿼리스트링을 가져온다.
        console.log("search : " + search);

        search = search.slice(1, search.length).split('&');
        // slice(시작, 끝) : 시작부터 끝 전까지의 복사본을 새로운 배열 객체로 반환.
        // split('구분자') : 구분자를 기준으로 여러 개의 문자열로 나눈다.
    ...
    // init method
    // emit : 서버에서 클라이언트에게, 클라이언트에서 서버에게 데이터를 보낼때 사용
    // on : 서버는 클라이언트에게, 클라이언트는 서버에게 데이터를 받을 때 사용
