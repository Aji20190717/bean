...
<body class="bg-gra-01">
							...
							<c:choose>
								<c:when test="${empty login }">
									<button onclick="loginAlarm()" class="btn btn-secondary">
										<i class="material-icons">&#xE147;</i> <span>Add New CS</span>
									</button>
								</c:when>
								<c:otherwise>
									<a href="csinsertform.do?csname=${login.member_name}"
										class="btn btn-secondary"><i class="material-icons">&#xE147;</i>
										<span>Add New CS</span></a>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
							...
							<c:otherwise>
								<c:forEach items="${list }" var="csdto">
									<tr>
										...
										<td>${csdto.csname }</td>
										<td><fmt:formatDate value="${csdto.csdate }"
	...
	<a onclick="chatbotStart('${login.member_name}');" class="float"> <i
		class="fa fa-envelope my-float"></i>
	</a>
