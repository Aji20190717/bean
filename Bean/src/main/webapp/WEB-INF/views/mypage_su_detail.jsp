...
<body>
	...
	<div>
		...
		<div>
			<span>회원 ID</span><span>${login.member_id}</span><br /> <span>회원
				email</span><span>${login.member_email}</span><br /> <span>회원 연락처</span><span>${login.member_phone}</span><br />
			<span>회원 주소</span><span>${login.member_post }</span> <span>${login.member_addr1 }</span>
			<span>${login.member_addr2 }</span><br /> <span>회원 생년월일</span><span>${login.member_birth}</span><br />
			<span> <c:choose>
					...
					<c:otherwise>
						<c:forEach items="${classList }" var="classDto">
							<tr>
								...
								<td><button value="class 탈퇴"
										onclick="location.href='studentRun.do?member_no=${login.member_no}&class_no=${classDto.class_no }'"></button></td>
							</tr>
						</c:forEach>
					</c:otherwise>
				</c:choose>
		...
		<div>
			<input type="button" value="정보수정"
				onclick="location.href='myinfoupdateform.do'" /><br /> <input
